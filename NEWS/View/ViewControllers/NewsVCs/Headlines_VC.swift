...
class Headlines_VC: GenericViewController<News_V> {
    
    let disposeBag = DisposeBag()
    lazy var viewModel: HeadlinesViewModel = {
        self.bindViewModel(viewModel: $0)
        return $0
    } (HeadlinesViewModel(httpService: HTTPService()))
...
extension Headlines_VC {
    func bindViewModel(viewModel: HeadlinesViewModel) {
        ...
        viewModel.basicDetails
            ...
            .subscribe(onNext: { [weak self] (basicDetail) in
                self?.contentView.userSelector.viewModel?.titlesArray   = basicDetail.selectorCategories
    ...
    func contentViewViewModelBinding() {
        ...
        contentView.userSelector.viewModel?.selectedButtonTitle
            ...
            .subscribe(onNext: { [weak self] (selectedCategory) in
                self?.viewModel.selectCategory(category: selectedCategory)
            })
            .disposed(by: disposeBag)
...
extension Headlines_VC: UICollectionViewDelegate, UITableViewDelegate {
